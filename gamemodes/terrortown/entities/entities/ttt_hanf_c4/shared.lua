...
if SERVER then
    ...
    local function ReceiveC4Config(ply, cmd, args)
        ...
        if IsValid(bomb) and bomb:GetClass() == "ttt_hanf_c4" and (not bomb:GetArmed()) then
    ...
    local function ReceiveC4Disarm(ply, cmd, args)
        ...
        if IsValid(bomb) and bomb:GetClass() == "ttt_hanf_c4" and not bomb.DisarmCausedExplosion and bomb:GetArmed() then
            if bomb:GetPos():Distance(ply:GetPos()) > 256 then
                return
    ...
    hook.Add("TTTRenderEntityInfo", "HUDDrawTargetIDC4", function(tData)
        ...
        if not IsValid(client) or not client:IsTerror() or not client:Alive()
            or not IsValid(ent) or tData:GetEntityDistance() > 100 or ent:GetClass() ~= "ttt_hanf_c4" then
            return
        end
